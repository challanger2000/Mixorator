...
void Controller::requestFinalSnapshot(std::uint64_t g) noexcept {if(g==0||g==requestedFinalGeneration_)return;auto* m=allocateMessage();if(!m)return;m->setMessageID(kRequestFinalSnapshotMessage);if(auto* a=m->getAttributes()){a->setInt(kFinalSnapshotGenerationKey,static_cast<Steinberg::int64_t>(g));if(sendMessage(m)==Steinberg::kResultTrue)requestedFinalGeneration_=g;}m->release();}
