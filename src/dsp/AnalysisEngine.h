...
public:
    ...
    std::uint64_t hardClippedSampleCount() const noexcept { return hardClippedSampleCount_.load(std::memory_order_relaxed); }
    std::uint64_t sampleOverCount() const noexcept { return sampleOverCount_.load(std::memory_order_relaxed); }
...
private:
    struct Biquad { double b0 {1.0}; double b1 {0.0}; double b2 {0.0}; double a1 {0.0}; double a2 {0.0}; double z1 {0.0}; double z2 {0.0}; double process(double x) noexcept; void clear() noexcept; };
    static constexpr std::size_t kFftSize = 1024;
    template <typename Sample> void processBlock(Sample* const* channels, int numChannels, int numSamples) noexcept;
    ...
    void pushWindowSample(std::vector<double>& ring, std::size_t& writeIndex, std::size_t& validSamples, double& sum, double value) noexcept;
    ...
    std::uint64_t hardClippedSampleCountRaw_ {0};
    std::uint64_t sampleOverCountRaw_ {0};
    ...
    std::atomic<std::uint64_t> hardClippedSampleCount_ {0};
    std::atomic<std::uint64_t> sampleOverCount_ {0};
